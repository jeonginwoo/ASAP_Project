...
from ..py_hanspell.hanspell import spell_checker # -> 각자 hanspell 설정에 맞춰 쓰기
import re
...
class Konlp():
    ...
    # 바꿀 단어들을 사전 형태로 정의
        ...
        self.queryName = {'헬로디아블로와퍼':'헬로_디아블로_와퍼', '헬로릴리트와퍼':'헬로_릴리트_와퍼','헬로이나리우스와퍼':'헬로_이나리우스_와퍼',
        ...
        self.ham = ['와퍼','치즈와퍼','불고기와퍼','갈릭불고기와퍼','콰트로치즈와퍼','통새우와퍼','롱치킨버거',
        ...
        self.side = ['너겟킹','치즈스틱','어니언링','바삭킹','쉐이킹프라이','크리미모짜볼','코코넛슈림프','21치즈스틱',
        ...
        self.drink = ['망고선데','초코망고선데','레드애플맛제로','레몬라임맛제로','컵망고아이스크림','아메리카노','아이스아메리카노','핫초코','아이스초코','코카콜라',
                    '코카콜라제로','스프라이트','스프라이트제로','미닛메이드오렌지','순수','순수(미네랄워터)','제로']
        ...
        self.ing = ['치즈','슬라이스치즈','슈레디드치즈','피클','할라피뇨','통새우','베이컨','양상추','양파','해쉬브라운','토마토','마늘','마늘칩']
        self.sau = ['마요','마요네즈','마요네즈소스','불고기','불고기소스','바베큐','바베큐소스','케찹','케찹소스','디아블로','디아블로소스',
            '매운토마토','매운토마토소스','치즈소스','타르타르','타르타르소스','베이컨잼','베이컨잼소스','머스타드','머스타드소스']
        self.pat = ['소고기','비프','새우','쉬림프','슈림프','치킨','닭','스테이크']
        self.etc = ['영양소','영양성분','알레르기','가격','정보','상세정보','맵기','맵','안맵','nutrition_info','allergy',
                    'N','A','price','info','spicy','높','많','낮','적','차갑','차','시원','따시','따듯','뜨겁','ice','hot']
        self.cat = ['M','S','DD','햄버거','버거','사이드','디저트','음료수','아이스크림','아침','점심','배고프','저녁','메뉴','음식','끼니','드시','먹']
        self.eve = self.ham+self.side+self.drink+self.neu+self.al+self.ing+self.sau+self.pat+self.etc+self.cat

    def replace_multiple_words(self,text):
        for old_word, new_word in self.replacements.items():
            text = text.replace(old_word, new_word)
        return text

    def toQuery(self,a):
        a = self.replace_multiple_words(a)
        m = self.kkma.morphs(a) # 형태소 추출
        n = self.kkma.nouns(a) # 명사만 추출
        ...
        tmp = '' # 임시로 이을 문장
        for i in re1: # 토큰
            if i in self.toQ.keys() : i = self.toQ[i]
        ...
        for i in range(len(re2)):
            ...
            for j in re2[i].split():
                if j in self.neu+self.cat+['A','N','or']:
                    ...
                    elif j in self.drink : what = 'DD'
                    if j in self.queryName: j = self.queryName[j]
